...
use sower\Db;
use sower\service\database\exception\BindParamException;
...
class Query
{
    ...
    public function count(string $field = '*'): int
    {
        if (!empty($this->options['group'])) {
            ...
            $subSql  = $this->options($options)->field('count(' . $field . ') AS Sower_count')->bind($this->bind)->buildSql();
    ...
    public function paginate($listRows = null, $simple = false, $config = [])
    {
        ...
        if (!isset($total) && !$simple) {
            ...
            $bind    = $this->bind;
            $total   = $this->count();
            $results = $this->options($options)->bind($bind)->page($page, $listRows)->dataset();
        } elseif ($simple) {
            $results = $this->limit(($page - 1) * $listRows, $listRows + 1)->dataset();
            $total   = null;
        } else {
            $results = $this->page($page, $listRows)->dataset();
        }
    ...
    {
        $name = $name ?: 'SowerBind_' . (count($this->bind) + 1) . '_';
    ...
    public function dataset($data = null)
    {
        ...
        $resultSet = $this->connection->dataset($this);
    ...
    {
        return $this->allowEmpty(true)->first($data);
    }
    ...
    {
        return $this->failException(true)->dataset($data);
    }
    ...
    {
        return $this->failException(true)->first($data);
    }
    ...
    public function chunk(int $count, callable $callback, $column = null, string $order = 'asc'): bool
    {
        ...
        $resultSet = $query->order($column, $order)->dataset();
        ...
        while (count($resultSet) > 0) {
            ...
            $resultSet = $query->bind($bind)->order($column, $order)->dataset();
        }
    ...
    {
        return $sub ? '( ' . $this->fetchSql()->dataset() . ' )' : $this->fetchSql()->dataset();
    }
