...
class Fetch
{
    ...
    public function value(string $field, $default = null, bool $one = true): string
    {
        ...
        // 生成查询SQL
        $sql = $this->builder->dataset($this->query, $one);
    ...
    public function column(string $field, string $key = ''): string
    {
        ...
        // 生成查询SQL
        $sql = $this->builder->dataset($this->query);
     ...
     * @return string
     */
    public function dataset($data = null): string
    {
        $this->query->parseOptions();
    ...
    public function first($data = null): string
    {
        ...
        // 生成查询SQL
        $sql = $this->builder->dataset($this->query, true);
    ...
    public function count(string $field = '*'): string
    {
        ...
        if (!empty($options['group'])) {
            // 支持GROUP
            $bind   = $this->query->getBind();
            $subSql = $this->query->options($options)->field('count(' . $field . ') AS Sower_count')->bind($bind)->buildSql();
