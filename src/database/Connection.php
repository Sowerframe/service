...
use PDOStatement;
use sower\Cache;
use sower\driver\cache\CacheItem;
use sower\Container;
use sower\Db;
...
abstract class Connection
{
    ...
    {
        return $this->getConfig('builder') ?: '\\sower\\service\\database\\builder\\' . ucfirst($this->getConfig('type'));
    }
     ...
     * @param \sower\Model $model     模型对象实例
    ...
    public function pdo(Query $query): PDOStatement
    {
        $bind = $query->getBind();
        // 生成查询SQL
        $sql = $this->builder->dataset($query);
     ...
     */
    public function first(Query $query): array
    {
        // 分析查询表达式
        $options = $query->parseOptions();
        ...
        // 生成查询SQL
        $sql = $this->builder->dataset($query, true);
     ...
     * @throws DataNotFoundException
     */
    public function dataset(Query $query): array
    {
        // 分析查询表达式
        $options = $query->parseOptions();
    ...
    public function value(Query $query, string $field, $default = null, bool $one = true)
    {
        ...
        // 生成查询SQL
        $sql = $this->builder->dataset($query, $one);
    ...
    protected function debug(bool $start, string $sql = '', bool $master = false): void
    {
        if (!empty($this->config['debug'])) {
            ...
            if ($start) {
                ...
                // SQL性能分析
                if ($this->config['sql_explain'] && 0 === stripos(trim($sql), 'dataset')) {
        ...
        if ($key instanceof CacheItem) {
            ...
            if (true === $key) {
                if (!empty($query->getOptions('key'))) {
                    $key = 'sower:' . $this->getConfig('database') . '.' . $query->getTable() . '|' . $query->getOptions('key');
                } else {
